...
resource "aws_cloudwatch_event_rule" "s3_object_created" {
  name        = "${var.project}-s3-object-created-${random_id.suffix.hex}"
...
resource "aws_cloudwatch_event_target" "s3_to_step_function" {
  ...
  target_id = "${var.project}-step-function-${random_id.suffix.hex}"
...
resource "aws_iam_role" "eventbridge_to_sfn" {
  name = "${var.project}-eventbridge-to-sfn-${random_id.suffix.hex}"
...
resource "aws_iam_role_policy" "eventbridge_to_sfn" {
  name = "${var.project}-eventbridge-to-sfn-policy-${random_id.suffix.hex}"
...
resource "aws_cloudwatch_log_group" "call_audio_insights_sfn" {
  name              = "/aws/vendedlogs/states/${var.project}-call-audio-to-insights-${random_id.suffix.hex}"
